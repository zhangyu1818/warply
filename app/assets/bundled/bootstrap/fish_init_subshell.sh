...
set _msg (printf "{\"hook\": \"InitShell\", \"value\": {\"session_id\": $WARP_SESSION_ID, \"user\": \"%s\", \"hostname\": \"%s\", \"shell\": \"fish\", \"is_subshell\": true}}" "$_user" "$_hostname" | command od -An -v -tx1 | command tr -d " \n")
