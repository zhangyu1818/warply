...
impl LocalDiffStateModel {
    ...
    pub(crate) async fn load_metadata_for_repo(
        repo_path: PathBuf,
        ...
        .await
        .unwrap_or_default();
        let upstream_ref = upstream_branch;
    ...
    async fn num_lines_for_untracked_entry(repo_path: &Path, file_path: &Path) -> usize {
    ...
    pub async fn diff_metadata_against_head(repo_path: &Path) -> Result<DiffMetadataAgainstBase> {
        ...
        for (file_path, status) in &changed_files {
            let (additions, deletions) = if let Some(metadata) = num_stat_metadata.get(file_path) {
                ...
                (
                    Self::num_lines_for_untracked_entry(repo_path, Path::new(file_path)).await,
                    0,
                )
            } else {
                (0, 0)
