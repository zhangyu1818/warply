...
impl RemoteDiffStateModel {
    ...
    pub fn get_main_branch_stats(&self) -> Option<DiffStats> {
        self.metadata
            .as_ref()
            .and_then(|m| m.against_base_branch.as_ref().map(|b| b.aggregate_stats))
    }
    ...
    pub fn get_stats_for_mode(&self, mode: DiffMode) -> Option<DiffStats> {
        match mode {
            DiffMode::Head => self.get_uncommitted_stats(),
            DiffMode::MainBranch => self.get_main_branch_stats(),
    ...
    pub fn active_repository_path(&self, _ctx: &warpui::AppContext) -> Option<std::path::PathBuf> {
        Some(self.remote_path.path.to_local_path_lossy())
    }

    pub fn is_inside_repository(&self) -> bool {
        !matches!(self.state, InternalRemoteDiffState::NotInRepository)
    }
