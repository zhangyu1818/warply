...
/// The top-level model for the remote terminal daemon.
///
...
impl ServerModel {
    pub fn new(ctx: &mut ModelContext<Self>) -> Self {
        ...
        }
        {
            ...
            ctx.subscribe_to_model(&diff_states, |me, update, _ctx| {
                me.handle_diff_state_update(update);
            });
        }
                            ...
                            let message = match (&content, server_version) {
                                (Some(content), Some(sv)) => {
                                    server_message::Message::OpenBufferResponse(
                                        OpenBufferResponse {
                                            result: Some(
                                                open_buffer_response::Result::Success(
                                                    OpenBufferSuccess {
    ...
    pub fn deregister_connection(&mut self, conn_id: ConnectionId, ctx: &mut ModelContext<Self>) {
        ...
        self.diff_states
            .update(ctx, |manager, _| manager.remove_connection(conn_id));
    ...
    fn handle_abort(&mut self, abort: Abort, request_id: &RequestId, ctx: &mut ModelContext<Self>) {
        ...
        if let Some(handle) = self.in_progress.remove(&target_id) {
            ...
            let found = self
                .diff_states
                .update(ctx, |manager, _| manager.abort_request(&target_id));
            if !found {
                log::info!(
                ...
                let result_oneof = match result {
                    Ok(output) => {
                        ...
                        if total > MAX_OUTPUT_BYTES {
                            ...
                            let stdout_max = (stdout.len() as f64 * ratio) as usize;
                            let stderr_max = (stderr.len() as f64 * ratio) as usize;
    ...
    fn handle_open_buffer(
        ...
        self.buffers
            .track_open_buffer(msg.path.clone(), file_id, buffer_state.buffer.clone());
