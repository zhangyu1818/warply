...
mod tests {
    ...
    fn filter_repository_update_by_prefix_keeps_only_matching_paths() {
        let watched_dir = PathBuf::from("/tmp/.warply-local/config");
        let other_dir = PathBuf::from("/tmp/.warply-local/worktrees/repo");
        let watched_file = watched_dir.join("profile.toml");
        let other_file = other_dir.join("README.md");

        let update = RepositoryUpdate {
            added: HashSet::from([
                TargetFile::new(watched_file.clone(), false),
        ...
        let filtered =
            filter_repository_update_by_prefix(&update, &watched_dir).expect("expected update");

        assert!(filtered.contains_added_or_modified(&TargetFile::new(watched_file, false)));
    ...
    fn filter_repository_update_by_prefix_converts_cross_boundary_moves() {
        let watched_dir = PathBuf::from("/tmp/.warp-local/config");
        let watched_file = watched_dir.join("profile.toml");
        let ignored_file = PathBuf::from("/tmp/.warp-local/worktrees/repo/profile.toml");
