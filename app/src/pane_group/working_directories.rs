...
impl WorkingDirectoriesModel {
    ...
    pub fn refresh_working_directories_for_pane_group(
        ...
        for (terminal_id, remote_path) in &remote_terminal_cwds {
            ...
            new_root_to_terminal.insert(remote_key.clone(), *terminal_id);
            new_remote_repo_roots.push(remote_key);
        }
        ...
        if let Some(focused_id) = focused_terminal_id {
            ...
            for (dir, terminal_id) in &new_root_to_terminal {
                if *terminal_id == focused_id {
                    if let Some(local_path) = dir.to_local_path() {
                        if let Some(repo_root) =
                            DetectedRepositories::as_ref(ctx).get_root_for_path(local_path)
                        {
                            let repo_root = LocalOrRemotePath::Local(repo_root);
