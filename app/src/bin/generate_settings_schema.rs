//! Generates a JSON Schema file describing Warply's user-facing settings.
//!
//! Usage:
//! ```
...
fn main() {
    ...
    root.insert(
        "title".to_string(),
        Value::String("Warply Settings".to_string()),
    );
    root.insert(
        "description".to_string(),
        Value::String(format!(
            "JSON Schema for Warply settings ({channel} channel, {entry_count} settings)"
        )),
    );
