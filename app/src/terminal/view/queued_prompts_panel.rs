...
impl QueuedPromptsPanelView {
    ...
    /// edit mode, and the queue has a head row.
    fn should_show_enter_hint(&self, ctx: &AppContext) -> bool {
        ...
        self.enter_sends_queued_prompt(ctx)
            ...
            && queue_model.queue(conv_id).first().is_some()
    }
    ...
    /// Updates each row's "send now" tooltip when a long-running-command subagent (the "full
    /// terminal use agent") takes or releases control.
    fn update_send_now_availability(&mut self, ctx: &mut ViewContext<Self>) {
        let Some(conv_id) = self.active_conversation_id else {
            return;
        };

        let rows: Vec<QueuedQueryId> = QueuedQueryModel::as_ref(ctx)
            .queue(conv_id)
            .iter()
            .map(|query| query.id())
            .collect();
        ...
        for query_id in &rows {
            ...
            let tooltip = if lrc_subagent_in_progress {
    ...
    pub(crate) fn commit_edit(&mut self, ctx: &mut ViewContext<Self>) {
        ...
        let Some(_) = self.editing_row_id(ctx) else {
            return;
        };
        let new_text = self
            .edit_editor
    ...
    fn handle_action(&mut self, action: &Self::Action, ctx: &mut ViewContext<Self>) {
        ...
        match action {
            ...
            QueuedPromptsPanelAction::DropEnd => {
                let Some(_) = self.dragging_query_id.take() else {
                    return;
                };
