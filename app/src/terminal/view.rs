...
use crate::ai::predict::prompt_suggestions::{
    has_pending_code_prompt_suggestion, is_accept_prompt_suggestion_bound_to_cmd_enter,
...
use crate::ai::{
    ...
    blocklist::{
        ...
        BlocklistAIInputModel, ConversationStatusUpdate, InputConfig, InputType, PRE_REWIND_PREFIX,
        PassiveSuggestionsModels, PendingQueryState, QueuedQueryId, QueuedQueryModel,
        QueuedQueryOrigin, ShellCommandExecutor, ShellCommandExecutorEvent,
        TerminalPassiveSuggestionsEvent, TerminalPassiveSuggestionsModel, ai_brand_color,
        ...
        is_lrc_auto_queue_active, summarization_cancel_dialog::SummarizationCancelDialog,
    },
...
use warpui::elements::{
    ...
    DropTarget, DropTargetData, Empty, EventHandler, Flex, NewScrollable, OffsetPositioning,
...
use block_banner::{WarpificationMode, render_warpification_banner};
...
use inline_banner::{
    ...
    SSHBannerState, VimModeBannerAction, render_alias_expansion_banner,
    render_inline_notifications_discovery_banner, render_inline_notifications_error_banner,
    render_inline_ssh_wrapper_banner, render_open_in_warp_banner,
...
/// Link to troubleshooting steps for SSH Warpification errors.
const CONTROLMASTER_ISSUES_URL: &str =
    "https://docs.warply.local/terminal/warpify/ssh#troubleshooting";
...
pub struct ExecuteCommandEvent {
    ...
    /// If the command was executed from a [`SavedWorkflow`], pass its ID here.
    pub workflow_id: Option<SyncId>,
    /// If the command was executed from a [`SavedWorkflow`] or WorkflowType::Local, store the
...
pub enum Event {
    ...
    // Tell the pane group to open the workflow modal with an existing saved workflow.
...
pub struct TerminalView {
    ...
    pending_env_var_collection: Option<SavedEnvVarCollection>,
    ...
    /// Available for any active local AI conversation.
...
impl TerminalView {
    ...
    pub fn new(
        ...
        let terminal_passive_suggestions_model = ctx.add_model(|ctx| {
            TerminalPassiveSuggestionsModel::new(
                active_session.clone(),
                model.clone(),
                ai_controller.clone(),
                &model_events_handle,
        ...
        ctx.subscribe_to_model(
            &terminal_passive_suggestions_model,
            Self::handle_terminal_passive_suggestions_event,
        );
        let passive_suggestions_models = PassiveSuggestionsModels {
            terminal: terminal_passive_suggestions_model,
        };
        ...
        // Conversation details panel for active AI conversations.
        ...
        let mut terminal_view = Self {
            ...
            local_identity: LocalIdentityProvider::as_ref(ctx).get().clone(),
            find_bar,
            resize_tx,
        ...
        } = event
        {
            ...
            if should_show_execute_plan_suggestion {
                if let Some(block) = self.last_ai_block() {
                    ...
                    self.on_terminal_prompt_suggestion_generated(
                        suggestion,
                        block_id,
                        "".to_string(),
                        0,
    ...
    fn drain_queued_prompts(
        ...
        match finish_reason {
            ...
            | FinishReason::CancelledDuringRequestedCommandExecution => {
                ...
                if !is_active_in_agent_view || !self.input.as_ref(ctx).buffer_text(ctx).is_empty() {
                    return;
                }
    ...
    /// Sends the leading prompts auto-queued during an agent-requested long-running command when
    ...
    pub(crate) fn send_lrc_queued_prompts(
        ...
        if editing_front_lrc_row {
            let Some(panel) = self.input.as_ref(ctx).queued_prompts_panel().cloned() else {
                return;
            };
            panel.update(ctx, |panel, ctx| panel.commit_edit(ctx));
        }
    ...
    fn handle_terminal_passive_suggestions_event(
        &mut self,
        _: ModelHandle<TerminalPassiveSuggestionsModel>,
        event: &TerminalPassiveSuggestionsEvent,
        ctx: &mut ViewContext<Self>,
    ) {
        match event {
            TerminalPassiveSuggestionsEvent::PromptSuggestionsGenerated {
                prompt_suggestion,
                block_id,
                command,
                request_duration_ms,
        ...
        match event {
            BlocklistAIHistoryEvent::AppendedExchange {
                ...
                if !should_add_ai_block {
                    ...
                    log::info!(
                        "ACP UI: skipped AI block insert conversation={:?} exchange={:?} task={:?} terminal={:?}",
                        conversation_id,
                        exchange_id,
                        task_id,
                        self.view_id,
                ...
                log::info!(
                    "ACP UI: inserting AI block conversation={:?} exchange={:?} stream={:?} terminal={:?} active_agent_view_conversation={:?} hidden={}",
                    conversation_id,
                    exchange_id,
                    response_stream_id,
                    self.view_id,
            ...
            BlocklistAIHistoryEvent::UpdatedStreamingExchange { exchange_id, .. } => {
                ...
                if let Some(ai_block) = self.rich_content_views.iter().find_map(|rich_content| {
                    rich_content
                        .ai_block_metadata()
                        .filter(|metadata| metadata.exchange_id == *exchange_id)
    ...
    fn fetch_and_update_conversation_details_panel(&mut self, ctx: &mut ViewContext<Self>) {
        let data = BlocklistAIHistoryModel::as_ref(ctx)
            .active_conversation(self.view_id)
            .map(|conversation| {
                crate::ai::conversation_details_panel::ConversationDetailsData::from_conversation(
                    conversation,
                    ctx,
                )
            });
    ...
    fn maybe_handle_ctrl_c_in_rich_content_block(&mut self, ctx: &mut ViewContext<Self>) {
            ...
            .is_some_and(|c| c.status().is_in_progress())
        {
            ...
            // This happens when a nested agent task (e.g., conversation search)
    ...
    fn handle_control_master_error(&mut self, ctx: &mut ViewContext<Self>) {
        ...
        if self.control_master_error_banner_state.associated_session_id != active_session_id {
            ...
            let _has_remote_server = active_session_id.is_some_and(|session_id| {
                self.sessions
                    .as_ref(ctx)
                    .get(session_id)
                    .is_some_and(|session| {
    ...
    fn handle_remote_warpification_is_unavailable(
        ...
        match &reason {
            WarpificationUnavailableReason::TmuxNotInstalled {
                ...
                let tmux_install_script = install_tmux_script(system_details, ctx);
                ...
                if tmux_install_script.is_some() || tmux_root_install_script.is_some() {
            ...
            WarpificationUnavailableReason::UnsupportedTmuxVersion { system_details } => {
                ...
                let tmux_install_script = install_tmux_script(system_details, ctx);
                if tmux_install_script.is_some() {
                    self.add_ssh_install_tmux_block(
                        system_details,
                        tmux_install_script,
                        None,
    ...
    fn add_ssh_install_tmux_block(
        ...
        tmux_install_script: Option<String>,
    ...
    fn add_bootstrap_success_block(
        ...
        let disable_tmux = matches!(warpification_source, WarpificationSource::Ssh) && {
    ...
    fn passive_code_diffs_enabled(ctx: &mut ViewContext<Self>) -> bool {
        ...
        let is_prompt_suggestions_enabled = ai_settings.is_terminal_prompt_suggestions_enabled();
    ...
    fn handle_terminal_event(&mut self, event: &ModelEvent, ctx: &mut ViewContext<Self>) {
        match event {
            ...
            ModelEvent::BlockCompleted(block_completed_event) => {
                ...
                if let BlockType::User(user_block_completed) = &block_completed_event.block_type {
            ...
            ModelEvent::AfterBlockCompleted(AfterBlockCompletedEvent {
                ...
                if let BlockType::User(block_completed) = block_type {
                    ...
                    if let Some(saved_workflow_id) = saved_workflow_id {
                        let id_and_type = CloudObjectTypeAndId::Workflow(*saved_workflow_id);
    ...
    fn init_project_and_suppress_banners(&mut self, path: PathBuf, ctx: &mut ViewContext<Self>) {
        log::info!("Running /init for new repo at {path:?}");
...
impl TerminalView {
    ...
    fn on_terminal_prompt_suggestion_generated(
        &mut self,
    ...
    /// size of the entire terminal (block_list + input OR alt-grid OR viewer loading) as its
    /// argument.
            ...
            for rich_content in self.rich_content_views.iter() {
                if let Some(ai_metadata) = rich_content.ai_block_metadata() {
                    ...
                    if ai_metadata.ai_block_handle.id() == *rich_content_view_id {
                        ...
                        if !ai_metadata.ai_block_handle.as_ref(ctx).is_restored() {
                            items.push(
    ...
    pub fn open_workflow_modal_with_existing(
        ...
        ctx.emit(Event::OpenWorkflowModalWithSavedWorkflow(workflow_id));
        ctx.notify();
    }
    ...
    fn toggle_input_hint_text(&mut self, ctx: &mut ViewContext<Self>) {
        let _new_val = InputSettings::handle(ctx)
    ...
    fn ask_ai(&mut self, ask_source: &AskAISource, ctx: &mut ViewContext<Self>) {
        ...
        let ask_data = match (ask_source, selection_string) {
            (AskAISource::SelectedTerminalText, Some(selection_string)) => {
            ...
            (AskAISource::Block { .. }, _) => {
                ...
                let block_index = match ask_source {
                    ...
                    AskAISource::SelectedTerminalText | AskAISource::SelectedBlocks => None,
                };
            ...
            (AskAISource::SelectedTerminalText, None) => return,
    ...
    fn handle_ai_block_event(
        ...
        match event {
            ...
            AIBlockEvent::OpenSettings => {
                ctx.emit(Event::OpenSettings(SettingsSection::AI));
            }
            #[cfg(feature = "local_fs")]
    ...
    fn finish_reason_for_conversation(
        ...
        self.ai_block_metadata_for_current_thread(&conversation_id, ctx)
            .next()
            .and_then(|metadata| metadata.ai_block_handle.as_ref(ctx).finish_reason())
    }
    ...
    fn ssh_banner_action(&self, action: SSHBannerAction, ctx: &mut ViewContext<Self>) {
        ...
        match action {
            LearnMore => {
                ctx.open_url("https://docs.warply.local/terminal/warpify/ssh#implementation");
            }
            Settings => {
                ctx.emit(Event::OpenSettings(SettingsSection::Warpify));
            }
    ...
    fn invoke_env_vars_in_current_session(
        ...
        let env_var_collection_id = env_var_collection.cloud_object_type_and_id();
        let env_var_collection_data = env_var_collection.model().string_model.clone();
        if let Some(session_id) = session_id {
            self.add_env_var_block_to_blocklist(
                env_var_collection_data
                    .title
                    .clone()
    ...
    fn invoke_env_vars_in_subshell(
        ...
        let env_var_collection_id = env_var_collection.cloud_object_type_and_id();
        let env_var_collection = env_var_collection.model().string_model.clone();
...
impl TypedActionView for TerminalView {
    ...
    fn handle_action(&mut self, action: &TerminalAction, ctx: &mut ViewContext<Self>) {
        ...
        match action {
            ...
            OpenWorkflowModalWithSavedWorkflow(workflow_id) => {
...
impl View for TerminalView {
    ...
    fn keymap_context(&self, app: &AppContext) -> warpui::keymap::Context {
        ...
        if self
            ...
            .prompt_suggestions_banner
            .is_some()
            || has_pending_code_prompt_suggestion(&model_lock, app)
        {
