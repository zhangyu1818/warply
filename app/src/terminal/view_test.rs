...
use crate::ai::blocklist::{
    ...
    BlocklistAIHistoryModel, InputConfig, InputType,
};
use crate::banner::{BannerEvent, DismissalType};
...
fn append_exchange_and_handle_event(
    ...
    let (conversation_id, task_id, exchange_id, response_stream_id) =
        history_model.update(ctx, |history_model, ctx| {
            ...
            let exchange = exchange_with_inputs(vec![input]);
...
fn initialize_agent_transcript_navigation_test(app: &mut App) {
...
fn cmd_up_in_agent_view_navigates_prompts_and_user_shell_blocks() {
    App::test((), |mut app| async move {
        initialize_agent_transcript_navigation_test(&mut app);
        let terminal = add_window_with_terminal(&mut app, None);
...
// Regression test for the "Seems like your completions are not working" banner: it must
// offer a permanent "Don't show me again" dismissal that is persisted, while the "x" close
// button keeps its existing per-session behavior.
#[test]
...
// Regression test: once the banner has been permanently dismissed it must not reopen.
#[test]
    ...
    App::test((), |mut app| async move {
        ...
        let handled = app
            .dispatch_keystroke(
                window_id,
                &[terminal.id()],
                &warpui::keymap::Keystroke::parse("cmd-enter").expect("valid keystroke"),
                false,
            )
            .expect("dispatch should succeed");
        assert!(handled, "cmd-enter should be handled from terminal context");
        ...
        terminal.read(&app, |view, ctx| {
            ...
            match block.agent_view_visibility() {
                AgentViewVisibility::Terminal {
                    ...
                    ..
                } => assert!(pending_conversation_ids.contains(&conversation_id)),
...
fn cmd_enter_from_active_non_empty_agent_view_requires_confirmation() {
    App::test((), |mut app| async move {
        ...
        let keystroke = warpui::keymap::Keystroke::parse("cmd-enter").expect("valid keystroke");

        let handled = app
            .dispatch_keystroke(window_id, &[terminal.id()], &keystroke, false)
            .expect("dispatch should succeed");
        assert!(handled, "new conversation keybinding should be handled");
