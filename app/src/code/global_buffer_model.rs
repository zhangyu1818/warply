...
impl GlobalBufferModel {
        ...
        ctx.spawn(
            async move {
                ...
                match response.result {
                    ...
                    None => Err("Remote server returned no OpenBuffer result".to_string()),
                }
            },
    ...
    fn apply_remote_open_response(
        &mut self,
        file_id: FileId,
        result: Result<remote_server::proto::OpenBufferSuccess, String>,
        ctx: &mut ModelContext<Self>,
    ) {
        match result {
            Ok(response) => {
    ...
    /// Re-open an existing remote buffer and reload its current server content.
    ...
    pub fn reopen_remote_buffer(&mut self, file_id: FileId, ctx: &mut ModelContext<Self>) {
        ...
        ctx.spawn(
            ...
            move |me, result, ctx| me.apply_remote_open_response(file_id, result, ctx),
        );
    }
