...
use crate::cloud_object::update_manager::{ObjectOperation, UpdateManager, UpdateManagerEvent};
use crate::http_api::{HttpApi, HttpApiProvider};
...
use warpui::{
    ...
    elements::{
        ...
        CrossAxisAlignment, Dismiss, DragAxis, Draggable, Element, Empty, Expanded,
...
const KEYBINDINGS_TO_CACHE: [&str; 1] = [TOGGLE_COMMAND_PALETTE_KEYBINDING_NAME];
...
impl Workspace {
    ...
    pub fn new(
        ...
        let http_api_provider = HttpApiProvider::as_ref(ctx);
        let http_api = http_api_provider.get();
    ...
    /// Notifies the agent views model that a terminal view gained focus.
    ...
    fn handle_ai_fact_view_event(&mut self, event: &AIFactViewEvent, ctx: &mut ViewContext<Self>) {
        match event {
            AIFactViewEvent::OpenSettings => {
                self.show_settings_with_section(Some(SettingsSection::AI), ctx);
            }
    ...
    fn unified_new_session_menu_items(
        ...
        // 7. Separator + New tab group entry.
    ...
    fn open_local_object_in_new_pane(&mut self, uid: &ObjectUid, ctx: &mut ViewContext<Self>) {
    ...
    pub fn open_workflow_from_intent(&mut self, workflow_id: SyncId, ctx: &mut ViewContext<Self>) {
        if ContextFlag::RunWorkflow.is_enabled() {
            match CloudModel::as_ref(ctx).get_workflow(&workflow_id).cloned() {
                ...
                None => self.add_tab_for_saved_workflow(workflow_id, ctx),
            }
        } else {
            self.add_tab_for_saved_workflow(workflow_id, ctx);
        }
    }
    ...
    fn setup_code_review_panel(
        ...
        if let Some((repo, diff_state_model)) = context_data {
            self.right_panel_view.update(ctx, |right_pane_view, ctx| {
                right_pane_view.open_code_review(repo.clone(), diff_state_model, ctx);
            });
        } else {
    ...
    fn tab_group_menu_items(
        ...
        let (pin_label, pin_action) = if self.tab_groups.get(&group_id).is_some_and(|g| g.pinned) {
    ...
    pub fn snapshot(
        ...
        let tab_groups: Vec<TabGroupSnapshot> = self
            .tab_groups
            .values()
    ...
    fn add_docker_sandbox_tab(&mut self, ctx: &mut ViewContext<Self>) {
        ...
        #[cfg(feature = "local_tty")]
        {
            // Resolve sbx via the user's interactive shell PATH so we find it
            // when installed via homebrew on Apple Silicon, `~/.local/bin`,
            // `nvm`-style paths, etc. This is async because capturing the
            // interactive PATH requires spawning the user's login shell.
            let window_id = ctx.window_id();
    ...
    fn add_tab_for_saved_workflow(&mut self, workflow_id: SyncId, ctx: &mut ViewContext<Self>) {
        let panes_layout = PanesLayout::Snapshot(Box::new(PaneNodeSnapshot::Leaf(LeafSnapshot {
            is_focused: true,
            custom_vertical_tabs_title: None,
            contents: LeafContents::Workflow(WorkflowPaneSnapshot::SavedWorkflow {
    ...
    fn fork_ai_conversation(
        ...
        let has_initial_query = initial_prompt.is_some();
        ...
        ctx.spawn(future, move |workspace, source_conversation, ctx| {
            let Some(RestoredConversationData::Conversation(source_conversation)) =
                source_conversation
            else {
    ...
    fn toggle_palette(
        ...
        // If another modal is open, don't show the palette since it won't be visible anyway.
    ...
    pub(crate) fn handle_file_tree_event(
        ...
        match event {
            ...
            pane_group::Event::OpenSavedWorkflowForEdit(workflow_id) => {
            ...
            pane_group::Event::InvokeEnvVarCollection {
                ...
                env_var_collection.as_saved_env_var_collection().clone(),
                *in_subshell,
                ctx,
            ),
            ...
            pane_group::Event::PaneFocused => {
                ...
                if self
                    ...
                    .active_session_view(ctx)
                    .is_some()
                    && self.active_tab_pane_group().as_ref(ctx).right_panel_open
                {
                    self.setup_code_review_panel(None, ctx);
                }
                    ...
                    match hovered_tab_index {
                        TabBarHoverIndex::BeforeTab {
                            ...
                            if let Some(pane) = pane {
                                ...
                                // pinned clamping for vertical tabs), so use them as-is.
            ...
            pane_group::Event::OpenLocalObjectInPane(uid) => {
                self.open_local_object_in_new_pane(uid, ctx);
            }
    ...
    fn show_command_search(
        ...
        if let Some(session_id) = self.active_session_id(ctx) {
            ...
            let ai_execution_context = session_context
                .as_ref()
                .map(|session_context| AiExecutionContext::new(&session_context.session));
    ...
    /// Runs a saved workflow in whichever input is currently active.
    fn run_saved_workflow_in_active_input(
        &mut self,
                ...
                match payload.as_ref() {
                    ...
                    AcceptWorkflow(accepted) => {
                        let (workflow, workflow_source) = match accepted {
                            AcceptedWorkflow::Saved { id, source } => {
                                let Some(saved_workflow) =
                                ...
                                (WorkflowType::Saved(Box::new(saved_workflow)), *source)
                            }
                            ...
                            || result.operation == ObjectOperation::Update
                        {
                            new_toast = new_toast.with_link(
                                ToastLink::new("View".to_string()).with_onclick_action(
                                    WorkspaceAction::ViewObject(CloudObjectTypeAndId::Workflow(
                                        workflow.id,
                                    )),
                                ),
                            )
    ...
    /// Opens an existing saved workflow.
    ...
    /// Adds the configurable right-side toolbar items plus fixed controls.
    ...
    fn render_panels(
        ...
        if self.current_workspace_state.is_right_panel_open() {
            let right_panel_content = if self.current_workspace_state.is_resource_center_open {
                ...
                log::warn!("is_right_panel_open() returned true, but resource center is not open");
                None
            };
    ...
    fn add_toggle_setting_context_flags(&self, app: &AppContext, context: &mut Context) {
        ...
        if ai_settings.is_terminal_prompt_suggestions_enabled() {
...
impl View for Workspace {
    ...
    fn render(&self, app: &AppContext) -> Box<dyn Element> {
        ...
        let event_handler =
            EventHandler::new(stack).on_modifier_state_changed(|ctx, _app, key_code, state| {
