...
Fork status: tab-close behavior for local tab configs may be retained, but app-managed instruction-bundle updates in this upstream spec are rejected in the local ACP-only fork. ACP agents own skills, and Warp must not ship skill instructions.
...
- Update the default template comments to show the new close behavior.
...
- The template comments should show both worktree variants:
  - remove the worktree only
  - remove the worktree and delete the branch
...
- Manual verification that the updated template example is internally consistent and produces valid TOML.
