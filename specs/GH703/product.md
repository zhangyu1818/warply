...
4. Logs, error toasts, and conversation titles do not leak Linear-originated prompt content.
...
2. **The prompt is never submitted to the LLM automatically.** No code path initiated by a Linear deeplink submits the prompt on the user's behalf. Specifically:
   ...
   - The prompt is not auto-sent through the macOS URL handler or a third-party app opener.
...
7. **No agent tools are invoked as a side effect of the deeplink.** Because nothing is submitted to the model until the user sends, no ACP/provider tool calls are triggered by the act of opening the URL.
8. **Local diagnostics reflect draft-only handling.** Local logs and state transitions should make the draft-only path debuggable without sending prompt content off-machine.
...
4. No model request, agent tool invocation, or model-facing network call is triggered by the act of handling the Linear deeplink on macOS.
5. No log line, toast, or conversation title contains the verbatim `prompt` body.
