...
- ACP AgentView conversations use the conversation title when available.
- Plugin-backed third-party CLI agent sessions use the latest user prompt ahead of title-like session metadata.

That makes the same vertical-tabs surface feel unpredictable. Users cannot rely on "agent conversation text" meaning the same thing across ACP AgentView and third-party agents, and users who prefer prompt-based labels do not have a single opt-in setting that applies to both.
...
- Apply the setting consistently to ACP AgentView conversations and plugin-backed third-party CLI agent sessions.
...
- Redesigning the vertical tabs settings UI beyond adding this setting in the appropriate Appearance / vertical tabs settings area.
...
Warp exposes a setting in the Appearance settings area near the vertical tabs controls.
...
- Description: `Show the latest user prompt instead of the generated conversation title for agent sessions in vertical tabs.`
...
- Non-terminal panes such as code panes, notebooks, workflows, settings, files, or local object panes.
...
For ACP AgentView conversations:
...
- Empty new conversations continue to use existing placeholder copy such as `New agent conversation`.
...
- ACP AgentView rows keep existing AgentView status behavior.
...
The setting only chooses the text used to identify the conversation; it does not alter whether a row is considered an ACP AgentView row, a third-party agent row, or a terminal row.
...
This fork stores the setting as a local vertical-tabs preference and does not use Warp cloud settings sync.

## Success criteria

1. A user can find a setting in the Appearance / vertical tabs settings area named `Use latest user prompt as conversation title in tab names`.
2. The setting is disabled by default.
3. With the setting disabled, an ACP AgentView conversation row in vertical tabs shows the generated conversation title when one exists.
...
5. With the setting enabled, an ACP AgentView conversation row in vertical tabs shows the latest user-authored prompt when one exists.
...
7. ACP AgentView and plugin-backed third-party CLI agent sessions follow the same setting semantics: disabled prefers title, enabled prefers latest user prompt.
...
9. Empty ACP AgentView conversations continue to show existing placeholder copy such as `New agent conversation`.
...
- **Default setting state**: Install or reset settings, open the Appearance / vertical tabs settings area, and verify `Use latest user prompt as conversation title in tab names` is disabled by default.
- **ACP AgentView title default**: Create an ACP AgentView conversation that receives a generated title. With vertical tabs enabled and the setting disabled, verify the row shows the generated title rather than the latest prompt text.
- **ACP AgentView prompt enabled**: Enable the setting, send a follow-up prompt in the same ACP AgentView conversation, and verify the vertical-tabs conversation text updates to the latest user prompt.
- **ACP AgentView empty conversation**: Start a new empty ACP AgentView conversation. Verify placeholder copy remains non-empty and appropriate.
...
- **Compact subtitle**: Configure compact vertical tabs so additional metadata shows command/conversation text. Toggle the setting and verify the subtitle changes consistently for ACP AgentView and plugin-backed third-party agent rows.
- **Detail sidecar**: Hover an eligible ACP AgentView row and an eligible third-party agent row. Verify the sidecar's command/conversation text follows the setting.
