...
Port local code-review UI improvements, including find/search behavior, save/edit behavior, discard controls, context-line comments, and editor state handling, as direct retained functionality. Do not restore code-review rollout gates, read-only fallback modes for old rollout states, unconnected embedded-comment prototypes, or old Agent SDK/cloud-agent routing to enable those UI paths.
