...
- Old Warp Agent service, SDK, cloud-agent, account, billing, orchestration, or backend-only behavior: reject.

An upstream setting is not missing merely because the field or page row survives. Classify its live runtime ownership and retain it only when it controls an ACP or local path. Settings whose ownership cannot be mapped to either path are legacy Warp Agent residue, not ACP settings.
...
Port local code-review UI improvements, including find/search behavior, save/edit behavior, discard controls, context-line comments, local git operations, and editor state handling, as direct retained functionality. Do not restore code-review rollout gates, read-only fallback modes for old rollout states, unconnected embedded-comment prototypes, old non-git-operation header fallbacks, or old Agent SDK/cloud-agent routing to enable those UI paths.
