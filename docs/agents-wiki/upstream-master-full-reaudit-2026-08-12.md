...
| 20 | Refresh changed local Markdown images | **Ported** | Applied prerequisite `a926a145f0` and feature commit `be547674a5`. The fork keeps the upstream local-file content-version pipeline through asset resolution, cache keys, decoded-image eviction, and source regression tests; no cloud or telemetry path was restored. |
...
| 22 | Fixed-height expandable toasts | **Ported** | Applied `c20645b9b0` from the upstream source, including the three-toast bound, oldest-toast eviction and timer cancellation, per-toast expansion state, truncation, accessible Show more/less controls, and source tests. The fork's existing `warpui_core` accessibility/ButtonVariant infrastructure made a prerequisite port unnecessary; upstream specs were omitted. |
