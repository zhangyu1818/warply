...
| 10 | Rich Input Ctrl+Enter setting | **Base restored; toggle pending** | Restored the retained CLI-agent Rich Input source ancestry that the fork deleted in `141123c3bd`; next apply `eaa936c78d` to that source instead of creating an independent input path. |
