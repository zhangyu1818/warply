...
- `4b39aa316` OSC 8 hyperlink support is a retained, feature-flagged terminal feature and should be ported in a focused follow-up. The cherry-pick touches 33 files (~1900 lines) and adds a per-grid hyperlink registry, OSC 8 parsing in the ANSI/grid handlers, Cmd+click routing, and new flat-storage/hyperlink types. It was deferred because of the breadth of the port and structural divergence: the fork's `app/src/features.rs` is a one-line re-export of `warp_core::features` (the upstream `enabled_features`/`init_feature_flags`/`OscHyperlinks` cfg wiring lives elsewhere), and multiple ANSI/grid/flat-storage modules conflicted. The upstream `specs/GH6393/` docs must be excluded when porting (rejected per merge discipline).
