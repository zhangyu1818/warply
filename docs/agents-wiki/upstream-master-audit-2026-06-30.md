...
- `38e765a09` Initially attempted to adapt the queued-prompt help-panel fix by adding a `QueuedPromptInlineEditorOpen` context guard, but reverted it: this fork has no queued-prompt inline editor, so the upstream context constant does not exist here and the guard is unnecessary. The `shift-?` binding remains as-is.
...
- `c96a018ad` Adapted the Copy-as-Markdown AI document action. The upstream `CreateWarpDriveNotebook`, `CopyLink`, `ShowInWarpDrive`, and `ObjectLinkCopied` telemetry paths were not ported; only the local Copy as Markdown action and its integration test were retained. The integration test helper was fixed to use this fork's 3-argument `start_new_conversation` signature.
