...
- Test app initializers that construct `TerminalView` must register `RemoteServerManager`, matching production startup. Do not treat SSH remote server as an optional cloud singleton.
...
- Removed mac-only `cfg` wrappers around the terminal-server worker, app menus, app icon setup, Services menu entry, and Sample Process action. These are now unconditional local macOS app behavior.
- Removed the Preview config-directory symlink migration and its unit/integration tests. The fork does not preserve historical Stable/Preview shared-config compatibility paths; config roots are created directly for the current local app.
- Removed the dead `preview_channel` Cargo feature and the Preview-only Universal Developer Input default branch. Input defaults now depend only on local PS1 settings and test/integration context, not on old channel rollout state.
- Removed the integration test that protected one-time migration of public settings from the platform-native store into `settings.toml`. Current settings storage is direct: retained public settings use the current TOML path, private settings stay in the native store, and old public native-store backfill should not be restored.
- Removed the `SettingsFile` rollout flag and `settings_file` Cargo feature. Public settings now initialize the TOML-backed backend directly, settings reload/open-file UI no longer has a disabled-feature branch, and tests that asserted native-store fallback were deleted.
- Removed unused local object model helpers and stale server/cloud-model wording from retained workflow, enum, generic-string-object, and local object update paths. `CloudModel` and `CloudObject` type names remain legacy local model names only; comments and logs should describe local persistence/object-model behavior.
- Removed the empty `app/src/ai/conversation_navigation/legacy.rs` source file. Conversation navigation remains in the current local ACP/history module; do not recreate old legacy navigation modules as merge placeholders.
- Removed old `AISettings` execution-profile compatibility fields under `agents.profiles.*`. The retained execution profile is stored directly in local user preferences, and permission speedbumps now write that profile instead of migrating or updating old settings keys.
...
- Removed mac-only `cfg` wrappers around settings import iTerm support, UserDefaults-backed preferences, notification sound settings, app-icon restart warning, resource limits, pane hover focus, Homebrew shell discovery, AgentView default shortcut selection, and Sample Process imports/dispatch.
...
- Removed the app build-script target OS/family branch, the local terminal-server Unix gate, the non-Apple jemalloc profiling symbol branch, and the Kitty shared-memory Unix gate. The app build path now assumes the retained macOS host and enables local filesystem, local TTY, and iTerm image support directly; SSH/remote-server OS detection remains separate.
- Removed the `channel_versions` crate and its dead Linux/Windows/Web target enum. Local block context now records macOS directly, while Warpify success snippets keep the existing POSIX rc-file path behavior without routing through channel-version platform overrides.
...
- Removed unused local-object helpers that exposed `CloudObjectTypeAndId` as server/cloud ids, and renamed the workflow info-box helper to use workflow id wording. Retained `SyncId::ServerId` remains only as the stable local id form documented above.
...
- Removed local static terminal Prompt Suggestion templates and their `static_prompt_suggestion_name` metadata. Terminal Prompt Suggestions must be generated by the user-configured OpenAI-compatible provider; retained local AgentView suggestions such as "Execute this plan" should remain ordinary local suggestions, not old static-template fallback paths.
...
- Removed the `DiffSetAsContext` rollout flag and `diff_set_as_context` Cargo feature. Adding file and diff-set context remains retained local code-review-to-ACP prompt-context behavior and no longer hides behind an old rollout-disabled branch.
- Removed the `AutoOpenCodeReviewPane` rollout flag and `auto_open_code_review_pane` Cargo feature. Auto-opening the local code review pane after the first agent change remains controlled by the retained user setting and existing local eligibility checks.
- Removed the `InlineCodeReview` rollout flag and `inline_code_review` Cargo feature. Inline code-review comments remain retained local code editor behavior and enter ACP AgentView through the existing inline-review origin without a disabled old-Agent branch.
- Removed the `PRCommentsSlashCommand` and `PRCommentsV2` rollout flags plus their Cargo features. `/pr-comments`, imported review comment rendering, outdated-comment handling, and open-in-code-review actions remain retained local code-review/ACP behavior; the old direct TerminalView insertion branch was removed instead of kept as a fallback.
- Removed the `ForkFromCommand` rollout flag and `fork_from_command` Cargo feature. `/fork-from` remains retained local AgentView conversation-fork UI and no longer has a disabled rollout branch.
- Removed the `InlineRepoMenu` rollout flag and `inline_repo_menu` Cargo feature. `/open-repo`, the indexed-repos inline menu, and restore-context hints remain retained local navigation behavior without a disabled old rollout branch.
- Removed the `CreateProjectFlow` rollout flag and `create_project_flow` Cargo feature. `/create-new-project` remains retained local ACP slash-command input behavior without a disabled rollout branch.
- Removed the `FileAndDiffSetComments` rollout flag and `file_and_diff_set_comments` Cargo feature. File- and diff-set-level code review comments remain retained local code-review UI behavior, with the disabled single-button fallback removed.
- Removed the `CodeReviewScrollPreservation` rollout flag. Code review scroll-context tracking and offset adjustment remain retained local editor behavior when file diffs update; integration tests no longer need to enable the rollout flag manually.
- Removed the `OpenWarpNewSettingsModes` and `MultiWorkspace` rollout flags plus their Cargo features. The current settings layout is now direct, and the old disabled-settings fallback widgets for external editor, code review button visibility, and auto-open-code-review-pane were removed. External editor runtime behavior remains retained local file-opening behavior; do not restore the old settings-mode fallback or dead cloud/workspace selection flag.
- Removed the dead `AllowOpeningFileLinksUsingEditorEnv` and `FileTree` rollout flags plus their Cargo features. Opening file links through editor configuration and the local file tree remain retained local functionality, but neither path is controlled by these old rollout shells.
- Removed the `ChangedLinesOnlyApplyDiffResult` rollout flag plus its Cargo feature. Accepted code-diff results now always return changed-line ranges to retained ACP/code-review flows instead of falling back to empty file-location ranges.
...
- Removed the unused conversation `run_id` persistence/event plumbing and assignment method. ACP sessions use protocol session IDs inside the ACP runtime, while local conversation persistence keeps conversation IDs plus ACP transcript JSON; do not restore old agent-run ID propagation.
- Removed stale AgentView renderable-error branches for provider overload and automatic network resume. ACP AgentView failures now render the current ACP runtime error message directly; OpenAI-compatible terminal suggestions keep their separate request error path and should not be routed through old AgentView provider-error branches.
- Removed Agent conversation time-to-first-token/time-to-last-token storage and update plumbing. ACP AgentView keeps local transcript content, status, model ids, and timing needed for display/restore; old provider latency metrics should not be persisted or reintroduced as telemetry fields.
- Removed stale remote-server feature-flag, cloud-update, enum-update-request, and local-object server-ownership wording from retained code comments. SSH remote server, Warpify, and legacy `ServerId` data support remain retained local/remote terminal functionality, not Warp service-client behavior.
...
- Reworded retained `remote_server` comments away from Warp-hosted download/install and account-auth language. The fork keeps the SSH remote-server extension path and retained ControlMaster-backed SSH fallback, while remote auto-install from Warp infrastructure remains disabled.
- Reworded stale cloud/server comments in retained terminal restoration, notebook local-object updates, TOML user preferences, and computer-use screenshot scaling. These are local persistence or local utility paths; do not restore cloud mode, cloud settings restore, or server-side Go computer-use coupling from upstream.
- Renamed shallow UI helpers away from cloud-object wording where the local schema type names were not part of the API surface: local-object toast helpers, local-object pane opening helpers, and the rules page global row model. Keep `CloudModel`, `CloudObject`, and `CloudObjectTypeAndId` only where they remain retained local object-model types.
- Renamed private view/model event handlers from `handle_cloud_model_event` to local-object wording. This keeps the retained `CloudModelEvent` type name for the local object model while avoiding new UI-layer cloud semantics.
- Renamed UI/flow-layer `cloud_model` local variables to `local_object_model` where they simply reference retained local object state. Do not use `cloud_model` as a new local variable name outside the retained `CloudModel` implementation/tests.
- Corrected retained notebook editor multi-selection paste coverage so the post-URL-paste cursor used for mixed text/code paste lands inside the code block. This is test coverage hygiene for local editor behavior, not a product-surface change.
- Removed unused old Agent citation variants for Warp documentation and arbitrary web pages, plus the unused Warp-documentation subagent type. Agent citations in the retained local flow now only point at local objects; do not restore old Warp Agent documentation/web citation branches unless ACP introduces a current protocol-native citation surface.
- Removed old Agent `WebSearch`/`WebFetch` output messages and their inline views. ACP search/fetch display is handled through protocol-native `AcpToolCall` rendering; do not restore app-side old-agent web search/fetch message branches.
- Removed the execution-profile `web_search_enabled` field and "Call web tools" profile editor switch. Web search/fetch permissions were old Warp Agent app-side gates; ACP-compatible providers expose tool behavior through ACP adapter configuration and protocol-native tool calls, not Warp execution profiles.
- Removed the no-target ACP prompt submission helper and internal ACP event cache. ACP runs now enter through an `AcpRunTarget` tied to AgentView conversation/history streams; do not restore standalone ACP prompt paths that bypass the retained local AgentView session lifecycle.
- Removed stale ACP `dead_code` suppressions and the unused permission-cancel helper. ACP permission handling remains through active protocol requests and selected options in AgentView; do not keep unused compatibility helpers just to preserve hypothetical old UI controls.
- Removed the unused Blocklist AI attachment-download directory setter/field that referenced old agent-driver-managed attachment downloads. Retained ACP prompt attachments are parsed into local request context and should not regain driver download-directory plumbing.
- Removed the empty Blocklist AI pending auto-resume handle map and abort sites. ACP AgentView uses explicit resume/cancel state; do not restore old network-connectivity auto-resume tasks or provider fallback recovery loops.
