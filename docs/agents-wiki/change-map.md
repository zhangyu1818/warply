...
- Removed the integration test that protected one-time migration of public settings from the platform-native store into `settings.toml`. Current settings storage is direct: retained public settings use the current TOML path when enabled, private settings stay in the native store, and old public native-store backfill should not be restored.
