...
- `upstream-master-full-reaudit-2026-08-12.md`: Full re-audit of all 1812 commits in `27f4933b8..69254d73db`, plus the two later upstream commits through `a1cc3a3df3` that were individually rejected/not applicable, under the local/provider-backed feature-preservation and upstream-source-fidelity rules. It supersedes blanket rejections based on absent feature flags, missing prerequisites, patch size, or architectural drift; records the missing feature stacks and corrects already-present false positives.
