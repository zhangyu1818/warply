...
- `upstream-master-audit-2026-08-24.md`: Incremental audit for `dc1077845f..upstream/master` (10 commits), including the retained settings schema generation moved into `warply dump-settings-schema` (standalone generator removed, fail-closed `SETTINGS_SCHEMA_EXECUTABLE`/`SOURCE` script inputs, corrected cargo-feature-backed flag coverage), the settings registration compile-time refactor (shared `(group, value)`-generic body + `SettingChangeEvent` trait, adapted to the de-cloud-synced settings crate), the partial `ai_types` crate port (`AIConversationId`/`AIAgentActionId`/`TaskId` with old-path re-exports), the completer option-argument by-value-position fix, the Warpified pwsh PSReadLine vi-mode corruption fix, and the rust-analyzer toolchain component, plus rejected multi-team AI-autonomy and Windows-arm64 release infra and not-applicable fork-owned CI workflow commits.
- `upstream-master-audit-2026-08-25.md`: Incremental audit for `79a9cb721a..upstream/master` (22 commits), including the retained ConPTY doc citation, two command-signatures bumps (`ac69f9b` → `d3725aa`, ln spec fix), the editor `EditDelta` Arc-wrap with borrowing layout (APP-4844), the last-tab shortcut hint precedence fix, the serde Content→JSON deserializer rewrite adapted to fork variant sets (`DProtoHook` 16 arms, `Artifact` envelope without EXTERNAL_REFERENCE, `AIAgentContext`/`AIAgentAttachment` tagged twins without cloud/skills variants), and the `/warp-agent-review` docs rename, plus rejected multi-team/skills/onboarding/Windows-CI commits and not-applicable TUI/MCP/agent-sdk/flag-promotion/fork-owned-CI commits.
- `upstream-master-audit-2026-08-26.md`: Incremental audit for `c5e4a02e3..upstream/master` (11 commits), including the retained release-cli codegen-units 1→4 build-profile bump and the settings-search `numbers` term for the code-editor line-numbers widget, plus the deferred terminal-grid crate split `21f413b79` with a recorded structural blocker (moved-file set diverged by fork removals; seam requires `warp_isolation_platform`/`session-sharing-protocol`/sentry/wasm infrastructure), and rejected multi-team/Teams-members/agent-environment-snapshot commits and not-applicable wasm-FTUE/MAA-teardown/STAKEHOLDERS/shared-session-test commits.
- `upstream-master-audit-2026-08-27.md`: Incremental audit for `1846f3000..upstream/master` (25 commits), including the retained editor rayon fan-out bound (APP-5392, chunked parallel layout + 2M-char per-line cap, adapted `report_error!` → `log::error!`), the bash bootstrap `shell_plugins` Bootstrapped-payload fix, the theme background image animation enablement, and the warping-row model naming partial port (ACP `model_info` display name, without the WarpingModelName rollout flag or Warp-server fallback machinery), plus six pre-existing `warp_editor` test failures fixed (stale expectations from the fork's URL localization), and rejected multi-team stack ×8, Teams/Windows/voice/TUI/skills/onboarding-deep-link/custom-inference-endpoints commits, and not-applicable cosmic-text pin (no fork dep; macOS CoreText unaffected) and `create_file` allow_overwrite (no live producer/consumer path).
- `upstream-master-audit-2026-08-28.md`: Incremental audit for `511b952c2..upstream/master` (10 commits), including the retained native shell completions feature for all four shells via in-band generator commands (CompletionSources dispatch, hex OSC payloads, replacement spans, flag wired default-on through the fork's cargo-feature pipeline), its integration tests (TranscriptScope→AgentViewState adaptation), Agent Mode user-query timestamps (tooltip anchored on the query container in the avatar-less fork renderer), and the secret_redaction/regex_dfas crate split applied to the fork's pre-split layout with SecretRange→StringRange, plus two pre-existing grid_handler_test failures fixed (stale column arithmetic from URL localization), and rejected multi-team P3c/docker-registry-managed-secret commits and not-applicable CI-bump/skills-lock/credits-toggle/share-modal commits.
- `upstream-master-audit-2026-08-29.md`: Incremental zero-port audit for `c2e4ee491..upstream/master` (8 commits), all rejected or not applicable: the CLIAgentSessionStatus→ConversationStatus bridge fix targets a function removed at fork creation with the ambient-agent stack (no `LocalAgentTaskSyncModel`/`AmbientAgentTaskId`/child-conversation anchors), and the Oz run_agents duplicate-pill fix, Factory MCP reconnect race, Windows oz CLI release artifact, SuperGrok connect Cancel, shared-session share_with_team_uid, mixed GitHub/GitLab code_forges, and factory-files webhook skill all touch only fork-absent removed surfaces.
- `upstream-master-audit-2026-08-31.md`: Incremental audit for `066ec71b7..upstream/master` (5 commits), including the adapted LRC liveness-signals port (`76cfd2c17`: local process-tree sampler on `ShellCommandExecutor` snapshots, `ShellProcessInfo` on TerminalModel, `LrcActivitySignal` wired through the cargo-feature pipeline, activity rendered into the ACP follow-up prompt at the `Display` boundary replacing upstream's proto wire path), plus rejected channel-gated-skill/computer-use changes and not-applicable upstream release-workflow and flag-promotion commits.
