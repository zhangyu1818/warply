...
Let's say we are interested in parsing the input `demo --color=auto`. Command calls generally have the form `cmd <arg1> <arg2> <argN>` where `<arg>`s are positional parameters. Let's use the primary functions of the parser and inspect. The first step we do is calling the tokenizer (the function `lex` here):

```rust
let input = "demo --color=auto";
let start_offset = 0;
...
let word3 = Span::new(5,17);

assert_eq!(word1.slice(input), "demo");
assert_eq!(word2.slice(input), " ");
assert_eq!(word3.slice(input), "--color=auto");
```
...
let example = Spanned { item: String::from("demo"), span: Span::new(0,4) };
assert_eq!(example.item, "demo".to_string());
assert_eq!(example.span, Span::new(0,4));

let example = String::from("demo").spanned(Span::new(0,4));
assert_eq!(example.item, "demo".to_string());
assert_eq!(example.span, Span::new(0,4));

let example = "demo -p --color=auto";

let full_span = Span::new(0, example.len());
let first_flag_span = Span::new(5,7);

assert_eq!(first_flag_span.slice(example), "-p");
assert_eq!(first_flag_span.until(full_span), Span::new(5,19));
assert_eq!(first_flag_span.until(full_span).slice(example), "-p --color=auto");

```

This is useful because as soon as the `lite` parse step happens. We get as output everything we need with `spans` correctly calculated. Let's go ahead and do a lite parse (the function `parse_tokens`) to our original example using as input the tokens generated by the lexer when processing the input `demo --color=auto`, like so:
...
let expected_word1 = String::from("demo").spanned(Span::new(0,4));
let expected_word2 = String::from("--color=auto").spanned(Span::new(5,17));
